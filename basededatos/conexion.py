...
#CREO LAS TABLAS DE LA BD
  ...
  def crearTabla(self):
    ...
    precio REAL,
    imagen TEXT,
    id_tipo_habitacion FOREIGN KEY REFERENCES tipo_habitacion(id_tipo_habitacion),
  ...
  def agregarEmpleado(self, empleado):
    self.cursor.execute(
        ...
        (empleado.getDni(), empleado.getNombre(), empleado.getApellido(),
         empleado.getTelefono(), empleado.getEmail(), empleado.getCategoria().getId()))
    self.conexion.commit()
  ...
  def agregarUsuario(self, usuario):
    self.cursor.execute(
        ...
        (usuario.getUsername(), usuario.getPassword(), usuario.getEmpleado().getId()))
    self.conexion.commit()
  ...
  def agregarHabitacion(self, habitacion):
    self.cursor.execute(
        ...
        (habitacion.getNumero(), habitacion.getCapacidad(),
         habitacion.getPrecio(), habitacion.getImagen(), habitacion.getTipo().getId(),
         habitacion.getEstado().getId()))
    self.conexion.commit()
  ...
  def modificarHabitacion(self, habitacion):
    self.cursor.execute(
        ...
        (habitacion.getNumero(), habitacion.getCapacidad(),
         habitacion.getPrecio(), habitacion.getImagen(), habitacion.getTipo().getId(),
         habitacion.getEstado().getId(),habitacion.getId()))
    self.conexion.commit()

  def eliminarHabitacion(self, habitacion):
    self.cursor.execute('''DELETE FROM habitacion WHERE id_habitacion=?''',(habitacion.getId()))
    self.conexion.commit()
  ...
  def eliminarTipoHabitacion(self, tipohabitacion):
    self.cursor.execute('''DELETE FROM tipo_habitacion WHERE id_tipo_habitacion=?''',(tipohabitacion.getId()))
    self.conexion.commit()
  ...
  def agregarEstado(self, estado):
    self.cursor.execute(
        '''INSERT INTO estado (descripcion,fin_de_servicio) VALUES(?,?))''',
        (estado.getDescripcion(),estado.getFechaFin()))
    self.conexion.commit()

  def modificarEstado(self, estado):
    self.cursor.execute(
        '''UPDATE estado SET descripcion=?, fin_de_servicio WHERE id_estado=?''',
        (estado.getDescripcion(),estado.getFechaFin(),estado.getId()))
    self.conexion.commit()

  def eliminarEstado(self, estado):
    self.cursor.execute('''DELETE FROM estado WHERE id_estado=?''',(estado.getId()))
    self.conexion.commit()
  ...
  def agregarReserva(self, reserva):
    self.cursor.execute(
        ...
        (reserva.getFechaEntrada(),reserva.getFechaSalida(),reserva.getCliente().getId(), reserva.getHabitacion().getId()))
    self.conexion.commit()
  ...
  def eliminarReserva(self, reserva):
    self.cursor.execute('''DELETE FROM reserva WHERE id_reserva=?''',(reserva.getId()))
    self.conexion.commit()
  ...
  def listarReservasActivas(self):
    self.cursor.execute('''SELECT * FROM reserva WHERE''')
    habitaciones = self.cursor.fetchall()
    return habitaciones
  ...
  def agregarCliente(self, cliente):
    self.cursor.execute(
        ...
        (cliente.getDni(),cliente.getNombre(),cliente.getApellido(),cliente.getTelefono(), cliente.getEmail()))
    self.conexion.commit()

  def modificarCliente(self, cliente):
    self.cursor.execute(
        '''UPDATE cliente SET dni=?,nombre=?,apellido=?,telefono=?,email=? WHERE id_cliente=?''',
        (cliente.getDni(),cliente.getNombre(),cliente.getApellido(),cliente.getTelefono(), cliente.getEmail(),cliente.getId()))
    self.conexion.commit()

  def eliminarCliente(self, cliente):
    self.cursor.execute('''DELETE FROM cliente WHERE id_cliente=?''',(cliente.getId()))
    self.conexion.commit()
  ...
  def agregarFactura(self, factura):
    self.cursor.execute(
        '''INSERT INTO factura(precio_total,fecha,id_metodo_pago,id_cliente,id_reserva) VALUES (?,?,?,?,?))''',(factura.getPrecioTotal(),factura.getFecha(),factura.getMetodoPago().getId(),factura.getCliente().getId(),factura.getReserva().getId()))
    self.conexion.commit()

  def modificarFactura(self, factura):
    self.cursor.execute(
        '''UPDATE factura SET precio_total=?,fecha=?,id_metodo_pago=?,id_cliente=?, id_reserva=? WHERE id_factura=?''',        (factura.getPrecioTotal(),factura.getFecha(),factura.getMetodoPago().getId(),factura.getCliente().getId(),factura.getReserva().getId(),factura.getId()))
    self.conexion.commit()

  def eliminarFactura(self, factura):
    self.cursor.execute('''DELETE FROM factura WHERE id_factura=?''',(factura.getId()))
    self.conexion.commit()
  ...
  def modificarMetodoPago(self, metodopago):
    self.cursor.execute(
        ...
        (metodopago.getNombre(),metodopago.getId()))
    self.conexion.commit()

  def eliminarMetodoPago(self, metodopago):
    self.cursor.execute('''DELETE FROM metodo_pago WHERE id_metodo_pago=?''', (metodopago.getId()))
    self.conexion.commit()
